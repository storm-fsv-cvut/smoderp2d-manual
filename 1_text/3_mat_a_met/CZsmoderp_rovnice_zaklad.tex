...
Tato kapitola nenese záměrně pouze název odtok ve vodních tocích. SMODERP je zamýšlen především jako nástroj pro navrhování opatření v ploše povodí. Cílem je simulovat a navrhovat odtoky i v dočasné hydrografické síti, která je tvořena přirozeným nebo častěji umělým přerušením přirozené odtokové dráhy s cílem zamezit soustředění odtoku a vzniku rýh. Nejčastěji se jedná o příkopy a průlehy které mají odváděcí a často erozní funkci. 
Všechny prvky (síť vodních toků, příkopy, průlehy, atp.) jsou zadávány v rámci jednoho shapefile. Každý jednotlivý úsek je zadán jako konkrétní polygon (feature). U jednotlivých úseků se předpokládá jejich napojení v uzlových bodech. Globálně model SMODERP pracuje v rastrové síti, ale v případě, že se na daném elemetu vyskytuje tok, je voda dále odváděna tímto tokem ve bez ohledu na směr odtoku daným modelem terénu.

\textbf{semka dát obrázek}
...
Pro vlastní výpočet je třeba zadat typ a příčný profil daného prvku. Délka úseku a sklonu jsou převzaty z vlastností polygonu. V případě, že budou zadány velmi krátké úseky (do pětinásobku rozlišení vstupního modelu terénu) může dojít k numerické nestabilitě výpočtu. 
Model určen pro malá povodí a tak jsou v modelu předpokládány pouze základní příčné profily (trojúhelník, obdélník, lichoběžník, parabola). Zadávání těchto příčných profilů není přímo součástí \textbf{shapefile}, ale pro ulehčení jsou parametry zadávány jako \textbf{samostatná tabulka} (\textit{viz. kapitola XXX}. V případě, že jsou některé charakteristiky shodné, je tak možné jim přiřadit shodné atributy z tabulky.
Zjednodušený výpočetní model neuvažuje rozlivy z koryta zpět do buněk, aby tvořili plošný odtok. Jednotlivé prvky narůstají podle zvolených parametrů, tak aby veškerá voda zůstala v korytě. Přehled parametrů je uveden v následující tabulce:
...
\item \textbf{Q365} - základní odtok. V případě dočasných prvků jako jsou příkopy je tato hodnota rovna nule, v případě vodních toků se jedná o základní odtok (např. $Q_{365}$)
\item \textbf{poznámky} - jedná se o volitelnou položku, do výpočtu se nijak nepropaguje
\end{itemize}

Tímto způsobem jsou zadány tvary prvků, které se v řešené lokalitě vyskytují.
\textbf{sem dát obrázek těch profilů}


\textbf{doplnit text jak probíhá vlastní výpočet} - tzn jak na sebe navazují jednotlivé úseky . a dát semka asi i nějaké  obrázky, jak to funguje. Je to v nějaké DP tuším (to najdu PK)
