...
\acro{es}[$es$]{intenzita srážky [$m/s$]}

\acro{hcrit}[$h^{crit}$]{kritická hloubka [$m$]}
...
\acro{I}[$I$]{sklon [@@@jj procenta nebo stupne]}
...
\acro{Oin}[$O^{in}$]{objem přítoku ze sousední buňky  za čas [$m^{3}/s$]}
...
\acro{Oout}[$O^{out}$]{objem odtoku z buňky  za čas [$m^{3}/s$]}
\acro{oout}[$o^{out}$]{výška odtoku z buňky  za čas [$m/s$]}
\acro{ooutrill}[$o^{out}_{rill}$]{výška odtoku v rýze za čas [$m/s$]}


\acro{Q365}[$Q365$]{základní odtok. V případě dočasných prvků jako jsou příkopy je tato hodnota rovna nule, v případě vodních toků se jedná o základní odtok. [$m^3/s$]}
...
\acro{ro}[$\rho$]{tíhové zrychlení @@@jj neni to hustota? zrychlení ma jine jednotky [$m s^{-1}$]}
...
\acro{tausur}[$\tau_{sur}$]{tečné napětí - povrchový odtok [$Pa$]}
