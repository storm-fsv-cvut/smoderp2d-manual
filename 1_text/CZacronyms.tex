...
\acro{hcrit}[$h_{crit}$]{kritická výška hladiny [$m$]}
...
\acro{Itot}[$I_{tot}$]{aktuální celkový přítok [$m^3/s$]}
...
\acro{n}[$n$]{Manningův součinitel drsnosti [$s/m^{1/3}$]}
...
\acro{PotI}[$I_{POT}$]{potenciální intercepce [$m$]}
...
\acro{Vout}[$V_{out}$]{Odteklý objem [$m^{3}$]}
